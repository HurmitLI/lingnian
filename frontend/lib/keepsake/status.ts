const KEEPSAKE_STATUS_LABELS: Record<string, string> = {
  queued: "已受理，等待本机开始合成",
  rendering: "正在这台 Mac 上合成视频",
  ready: "原声视频已生成",
  failed_retryable: "本机合成遇到问题，可以重试",
...
export function keepsakeErrorLabel(errorCode: string | null): string {
  if (!errorCode) return "本机合成没有完成，原始故事、录音和照片都不会被删除。";
  return KEEPSAKE_ERROR_LABELS[errorCode] ?? "本机合成没有完成，原始素材仍然安全保留。";
}
