...
export default function MemoryHubApp() {
  ...
  async function downloadProductionPackage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      saveBlob(result.blob, result.filename);
      setNotice("制作包已经下载：包含原始素材、故事原文、授权清单和待人工复核分镜；没有自动提交到生成服务。");
    } catch (value) {
      showError(value);
    } finally {
      setBusy(false);
      ...
      <main className="workspace-main memory-hub-main">
        <header className="workspace-header memory-hub-header">
          ...
          <div className="workspace-controls">
            ...
            <span className="privacy-badge">{IS_FORMAL_CLOUD ? "云端加密家庭档案" : "本机家庭档案"}</span>
          </div>
        </header>
        ...
        {!loading && selectedProfile && tab === "studio" && (
          <section className="card generation-studio">
            <div className="section-heading"><span>06</span><div><h2>把回忆做成影片</h2><p>保留采访原声，让画面进入讲述中的场景。生成后可以回看、下载和逐项验收。</p></div></div>
