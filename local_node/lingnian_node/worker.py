...
class ProductionWorker:
    def __init__(self, *, api_base: str, token: str, root: Path, comfy_root: Path, poll_seconds: int=8, cloud: CloudClient | None=None):
        self.token=token; self.root=root; self.comfy_root=comfy_root
    ...
    def heartbeat(self):
        stats=self.comfy.health(); device=stats["devices"][0]
    ...
    def _process(self, task: WorkerTask) -> None:
        ...
        try:
            ...
            if manifest["generation_type"]!=task.generation_type: raise ValueError("任务类型与素材包不一致。")
            self.cloud.progress(task,10,"素材已安全下载并校验")
            self.store.set(task.id,"generating"); self._status("generating","生成中",task_id=task.id)
            result=self._generate(task,manifest,work/"package")
            self.cloud.progress(task,90,"生成完成，正在校验结果")
            self.store.set(task.id,"uploading"); self._status("uploading","上传中",task_id=task.id)
            self.cloud.upload_result(task,result,actual_cost_cents=0)
            self.store.set(task.id,"succeeded"); self._status("succeeded","成功",task_id=task.id)
        except httpx.HTTPStatusError as exc:
            self.store.set(task.id,"failed",f"HTTP {exc.response.status_code}")
            self._status("failed","失败",task_id=task.id,error="云端拒绝了任务状态，任务将由云端恢复。")
            if exc.response.status_code not in {404,409}: self._safe_fail(task,"COMFYUI_TEMPORARY_FAILURE","云端通信暂时失败。",True)
    ...
    def _safe_fail(self, task: WorkerTask, code: str, message: str, retryable: bool) -> None:
        try: self.cloud.fail(task,code,message,retryable)
        except Exception: pass

    def _generate(self, task: WorkerTask, manifest: dict, package_root: Path) -> Path:
        if task.generation_type!="photo_restore": raise FileNotFoundError("当前节点尚未安装已授权视频模型。")
        media=next((m for m in manifest["media"] if m.get("mime_type","").startswith("image/")),None)
        if not media: raise ValueError("修复任务缺少照片。")
        ...
        try:
            ...
            if not images: raise RuntimeError("ComfyUI 没有返回输出文件。")
            item=images[0]
            result=self.comfy_root/"output"/item.get("subfolder","")/item["filename"]
            if not result.is_file(): raise RuntimeError("ComfyUI 输出文件不存在。")
            return result
        finally:
    ...
    def run_forever(self) -> None:
        ...
        while True:
            try:
                self.heartbeat(); self.once(); backoff=3; time.sleep(self.poll_seconds)
